...
# The process is launched by opentelemetry-instrument. Its distro configures the
# provider/exporter and automatically instruments FastAPI and HTTPX. Keep only
# business-specific span enrichment and the explicitly simulated datastore span.
# An API tracer is a proxy until the launcher installs the provider; importing
# this module alone neither installs middleware nor starts exporter threads.
...
async def call_downstream(name: str, payload: WorkRequest) -> dict[str, Any]:
    ...
    of the call. HTTPX auto-instrumentation injects trace context so the child span
